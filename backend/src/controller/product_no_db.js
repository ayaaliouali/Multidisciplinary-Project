...
const uniqueProductsData = Array.from(
  new Map(AllProductsData.map(item => [item.id, item])).values()
);

export const getAllProducts = async (req, res) => {
  try {
    const { pick, random } = req.query;
    let result = [...uniqueProductsData];
...
export const getTopProducts = async (req, res) => {
  try {
    const result = uniqueProductsData.filter(product => product.rating > 4.5);
    res.json(result);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
